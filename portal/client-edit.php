...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // B. Update/Create Client Account
        if (!empty($username)) {
            // Check if account exists
            $stmt_chk = $db->prepare("SELECT account_id FROM client_accounts WHERE client_id = ?");
            ...
            if ($exists) {
                ...
                if (!empty($password)) {
                    ...
                    $stmt_acc = $db->prepare("UPDATE client_accounts SET username = ?, password_hash = ? WHERE client_id = ?");
                    $stmt_acc->execute([$username, $hash, $client_id]);
                } else {
                    // Update Username only
                    $stmt_acc = $db->prepare("UPDATE client_accounts SET username = ? WHERE client_id = ?");
                    $stmt_acc->execute([$username, $client_id]);
                }
            } else {
                // Create new account if none existed
                if (!empty($password)) {
                    $hash = password_hash($password, PASSWORD_DEFAULT);
